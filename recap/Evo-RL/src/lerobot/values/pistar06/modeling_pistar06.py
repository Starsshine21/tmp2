...
def _load_language_model(
    ...
    local_files_only = repo_path.exists()
    repo_ref = str(repo_path.resolve()) if local_files_only else repo_id

    model_config = AutoConfig.from_pretrained(repo_ref, revision=revision, local_files_only=local_files_only)
...
class Pistar06Model(nn.Module):
    def __init__(self, cfg: Pistar06Config):
        ...
        vision_path = Path(cfg.vision_repo_id)
        vision_local = vision_path.exists()
        vision_ref = str(vision_path.resolve()) if vision_local else cfg.vision_repo_id

        self.vision_encoder = AutoModel.from_pretrained(
            vision_ref,
            revision=cfg.vision_revision,
            torch_dtype=self.model_dtype,
            local_files_only=vision_local,
        )
