...
On this machine, the existing conda environment `robosuite` is already usable for this repository and the project is installed in editable mode. You can activate it with:

```bash
source scripts/activate_recap_env.sh
```

This script also sets `NUMBA_DISABLE_JIT=1`, which avoids a local `robosuite` import issue caused by numba cache resolution in the current editable install.
