...
def run_value_inference_pipeline(
    ...
    if accelerator.is_main_process:
        ...
        if os.environ.get("LEROBOT_VALUE_INFER_DRY_RUN", "0") == "1":
            dry_run_path = output_dir / "acp_annotations.npz"
            np.savez_compressed(dry_run_path, absolute_indices=absolute_indices, **columns)
            logging.info("Dry run: wrote annotations to %s without modifying the dataset", dry_run_path)
        else:
