...
PRETRAINED_MODEL_PATH = os.environ.get(
    "DEXJOCO_RECAP_PRETRAINED_MODEL_PATH", config["pretrained_model_path"]
)
PRETRAINED_MODEL_ACTION_DIM_44_PATH = config["pretrained_model_action_dim_44_path"]
DATASET_ROOT = Path(os.environ.get("DEXJOCO_RECAP_DATASET_ROOT", config["dataset_root"]))
RAND_FULL_DATASET_ROOT = Path(config["rand_full_dataset_root"])
CKPTS_ROOT = Path(os.environ.get("DEXJOCO_RECAP_CKPTS_ROOT", config["ckpts_root"]))
FULL_CKPTS_ROOT = Path(
    os.environ.get(
        "DEXJOCO_RECAP_FULL_CKPTS_ROOT",
        str(CKPTS_ROOT.parent / "actor_full_checkpoints"),
    )
)
...
FULL_PYTORCH_COMPILE_MODE = os.environ.get(
    "DEXJOCO_RECAP_PYTORCH_COMPILE_MODE", "max-autotune"
)
...
def make_click_mouse_full_config():
    """True full-parameter PI0.5 config: no LoRA modules and no frozen weights."""
    from .config import DataConfig  # noqa: PLC0415
    from .config import SingleArmDataConfig  # noqa: PLC0415
    from .config import TrainConfig  # noqa: PLC0415
    ...
    model = pi0_config.Pi0Config(
        ...
        max_token_len=int(os.environ.get("DEXJOCO_RECAP_FULL_MAX_TOKEN_LEN", "128")),
        pytorch_compile_mode=(
            None
            if FULL_PYTORCH_COMPILE_MODE.lower() in {"none", "off", "0"}
            else FULL_PYTORCH_COMPILE_MODE
        ),
    )
    return TrainConfig(
        name="click_mouse_recap_full",
        exp_name="click_mouse_recap_full" + datetime.now().strftime("%Y%m%d"),  # noqa: DTZ005
        model=model,
        data=SingleArmDataConfig(
            root=Path(f"{DATASET_ROOT}/click_mouse"),
            repo_id="local_repo",
            base_img_name="observation.images.ego_right",
            base_config=DataConfig(prompt_from_task=True),
        ),
        batch_size=int(os.environ.get("DEXJOCO_RECAP_FULL_BATCH_SIZE", "32")),
        num_workers=int(os.environ.get("DEXJOCO_RECAP_NUM_WORKERS", "0")),
        lr_schedule=_optimizer.CosineDecaySchedule(
            warmup_steps=int(os.environ.get("DEXJOCO_RECAP_WARMUP_STEPS", "1000")),
            peak_lr=2e-5,
            decay_steps=int(os.environ.get("DEXJOCO_RECAP_FULL_TRAIN_STEPS", "30000")),
            decay_lr=2e-6,
        ),
        optimizer=_optimizer.AdamW(clip_gradient_norm=1.0),
        ...
        pytorch_weight_path=os.environ.get(
            "DEXJOCO_RECAP_FULL_PYTORCH_WEIGHT_PATH",
            "/nfs_global/S/yangrongzheng/evo-RL/dexjoco/checkpoints/pi05_pytorch/click_mouse/model.safetensors",
        ),
        pytorch_training_precision="bfloat16",
        num_train_steps=int(os.environ.get("DEXJOCO_RECAP_FULL_TRAIN_STEPS", "30000")),
        save_interval=int(os.environ.get("DEXJOCO_RECAP_SAVE_INTERVAL", "5000")),
        keep_period=int(os.environ.get("DEXJOCO_RECAP_KEEP_PERIOD", "10000")),
        wandb_enabled=WANDB_ENABLED,
        checkpoint_base_dir=str(FULL_CKPTS_ROOT),
    )
...
def get_dexjoco_configs():
    ...
    cfgs.append(make_click_mouse_full_config())
    return cfgs
