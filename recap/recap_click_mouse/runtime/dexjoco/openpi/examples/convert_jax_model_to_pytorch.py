...
def slice_initial_orbax_checkpoint(checkpoint_dir: str, restore_precision: str | None = None):
    ...
    # DexJoCo PI0.5 checkpoints are LoRA fine-tunes.  The PyTorch PI0 model has
    # dense layers only, so fold every trained LoRA pair into its base kernel
    # before doing the normal JAX -> PyTorch name/layout conversion.
    flat_params = traversals.flatten_mapping(params, sep="/")
    lora_a_keys = [key for key in flat_params if key.endswith("/lora_a") or key.endswith("_lora_a")]
    for lora_a_key in lora_a_keys:
        if lora_a_key.endswith("/lora_a"):
            prefix = lora_a_key.removesuffix("/lora_a")
            lora_b_key = f"{prefix}/lora_b"
            base_key = f"{prefix}/w"
        else:
            prefix = lora_a_key.removesuffix("_lora_a")
            lora_b_key = f"{prefix}_lora_b"
            base_key = prefix
        flat_params[base_key] = flat_params[base_key] + np.einsum(
            "...ir,...ro->...io", flat_params[lora_a_key], flat_params[lora_b_key]
        )
        del flat_params[lora_a_key]
        del flat_params[lora_b_key]
    params = traversals.unflatten_mapping(flat_params, sep="/")
...
def convert_pi0_checkpoint(
    ...
    # LoRA has been merged above; instantiate a genuinely dense/full model.
    dense_model_config = dataclasses.replace(
        model_config,
        paligemma_variant=model_config.paligemma_variant.replace("_lora", ""),
        action_expert_variant=model_config.action_expert_variant.replace("_lora", ""),
    )
    pi0_model = openpi.models_pytorch.pi0_pytorch.PI0Pytorch(dense_model_config)
    ...
    config_dict = {
        ...
        "paligemma_variant": dense_model_config.paligemma_variant,
        "action_expert_variant": dense_model_config.action_expert_variant,
        "precision": precision,
    }
