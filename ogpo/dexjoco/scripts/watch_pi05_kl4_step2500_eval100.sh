...
CHECKPOINT_DIR=${ROOT}/outputs/ogpo/checkpoints/click_mouse_pi05_jax_flash_td10_tgr_b8_8gpu_base_frozen_critic_kl4_resume500_10000/step_2500
CHECKPOINT=${CHECKPOINT_DIR}/click_mouse_pi05_jax_flash_td10_tgr_b8_8gpu_base_frozen_critic_kl4_resume500_10000_final.pt
EVAL_SCRIPT=${ROOT}/scripts/pi05_jax_ogpo_eval_100.slurm
EVAL_OUTPUT_DIR=${ROOT}/outputs/pi05_rollouts/click_mouse_kl4_step2500_seed27_100ep
STATE_DIR=${ROOT}/outputs/ogpo/watchdog
STATE_FILE=${STATE_DIR}/pi05_kl4_step2500_seed27_eval100.submitted
LOCK_FILE=${STATE_DIR}/pi05_kl4_step2500_seed27_eval100.lock
...
started_at=$(date +%s)
echo "Watching for complete step-2500 checkpoint: ${CHECKPOINT}"
while ! checkpoint_is_complete; do
  now=$(date +%s)
  if (( now - started_at >= TIMEOUT_SECONDS )); then
    echo "Timed out after ${TIMEOUT_SECONDS}s waiting for step 2500." >&2
    exit 1
  fi
  sleep "${POLL_SECONDS}"
done
...
eval_job_id=$(
  sbatch --parsable \
    --job-name=pi05_kl4_2500_eval100 \
