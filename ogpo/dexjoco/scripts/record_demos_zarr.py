...
def _write_demo_zarr_and_videos(
    ...
    for step_idx, transition in enumerate(trajectory):
        ...
        successes.append(bool(info.get("succeed", False)) if isinstance(info, dict) else False)
    ...
    # Keep synchronized RGB observations in replay.zarr for PI0.5 actor
    # training. MP4 files below remain a visualization-only derivative.
    for camera_key in camera_keys:
        frames = [
            _safe_squeeze_image(step.get("observations", {}).get(camera_key))
            for step in trajectory
        ]
        if all(frame is not None for frame in frames):
            try:
                episode_data[camera_key] = np.stack(frames, axis=0)
            except ValueError:
                print(f"[Replay warning] skipped inconsistent RGB stream: {camera_key}")
...
def main(_argv):
    ...
    while success_count < success_needed:
        ...
        transition = copy.deepcopy(
            dict(observations=obs, actions=actions, rewards=rew, dones=done, infos=info)
        )
