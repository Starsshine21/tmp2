...
def _patch_lerobot_column_stack_compat() -> None:
    """Handle datasets columns returned by newer Hugging Face datasets versions."""
    if getattr(lerobot_dataset.LeRobotDataset, "_openpi_column_stack_compat", False):
        return
    ...
    def _query_hf_dataset(self, query_indices: dict[str, list[int]]) -> dict:
        ...
        for key, q_idx in query_indices.items():
            ...
            if torch.is_tensor(values):
                ...
                result[key] = torch.stack(tuple(values))
        return result
...
def create_torch_dataset(
    ...
    episodes = None
    if max_episodes := os.getenv("OPENPI_MAX_EPISODES"):
        max_episodes_int = int(max_episodes)
        episodes = sorted(dataset_meta.episodes)[:max_episodes_int]
        logging.info("Using OPENPI_MAX_EPISODES=%d episodes=%s", max_episodes_int, episodes)
